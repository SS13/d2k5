...
/mob/living/carbon/human/New()
	...
	spawn (1)
		if(mutantrace == "zombie")
			if(!stand_icon)
				stand_icon = new /icon('zombie.dmi', "body_[g]_s")
			if(!lying_icon)
				lying_icon = new /icon('zombie.dmi', "body_[g]_l")
		else
			if(!stand_icon)
...
/mob/living/carbon/human/movement_delay()
	...
	if (istype(loc, /turf/simulated/floor/spacedome/water)) return 3 // It's hard to be slowed down in space by... anything
	...
	if(health_deficiency >= 40)
		...
		if(prob(1))
			if(!sleeping)
				src << "\red You are in pain!"
				if(prob(5))
					src << "\red The pain feels too intense!"
					emote(pick("twitch", "twitch_s", "blink", "faint", "gasp", "groan", "collapse", "scream"))
	...
	if (mutations & 256) // gotta go fast mut
		tally -= 3.0
...
/mob/living/carbon/human/update_clothing()
	...
	if (mutations & 16384)
		overlays += image("icon" = 'genetics.dmi', "icon_state" = "elect[!lying ? "_s" : "_l"]")

	if (mutations & 4096)
		overlays += image("icon" = 'genetics.dmi', "icon_state" = "goatmut[!lying ? "_s" : "_l"]")

	//Human Overlays
	if(druggy)
		overlays = image("icon" = 'statuseffects.dmi', "icon_state" = "druggy[!lying ? "_s" : "_l"]", "layer" = MOB_LAYER+0.5)

	if(confused >= 5 || dizziness >= 5)
		overlays += image("icon" = 'statuseffects.dmi', "icon_state" = "drunk[!lying ? "_s" : "_l"]", "layer" = MOB_LAYER+0.3)

	if(dizziness >= 50)
		overlays += image("icon" = 'statuseffects.dmi', "icon_state" = "greensick[!lying ? "_s" : "_l"]", "layer" = MOB_LAYER)
	else if(dizziness >= 80)
		overlays += image("icon" = 'statuseffects.dmi', "icon_state" = "yellowsick[!lying ? "_s" : "_l"]", "layer" = MOB_LAYER)
	else if(dizziness >= 100)
		overlays += image("icon" = 'statuseffects.dmi', "icon_state" = "redsick[!lying ? "_s" : "_l"]", "layer" = MOB_LAYER)

	if(turnedon)
		overlays += image("icon" = 'statuseffects.dmi', "icon_state" = "blush[!lying ? "_s" : "_l"]", "layer" = MOB_LAYER+0.3)

	if(confused >= 20 || dizziness >= 20)
		overlays += image("icon" = 'statuseffects.dmi', "icon_state" = "blackeyecircle[!lying ? "_s" : "_l"]", "layer" = MOB_LAYER+0.3)

	if(brainloss >= 60)
		overlays += image("icon" = 'statuseffects.dmi', "icon_state" = "mouthfoam[!lying ? "_s" : "_l"]", "layer" = MOB_LAYER+0.3)

	if(sleeping)
		overlays += image("icon" = 'statuseffects.dmi', "icon_state" = "zzz", "layer" = MOB_LAYER+1)
	//Human Overlays
	...
	if (w_uniform)
		...
		if (istype(w_uniform, /obj/item/clothing/under))
			...
			if (src.w_uniform.poop_covering)
				var/icon/poopstain_icon = icon('pooeffect.dmi', "uniformpoo[!src.lying ? "" : "2"]")
				src.overlays += image("icon" = poopstain_icon, "layer" = MOB_LAYER)
			if (src.w_uniform.egg_covering)
				var/icon/eggstain_icon = icon('eggeffect.dmi', "uniformegg[!src.lying ? "" : "2"]")
				src.overlays += image("icon" = eggstain_icon, "layer" = MOB_LAYER)
		else if (src.smearedwithpoo)
			var/icon/poopstain_icon = icon('pooeffect.dmi', "uniformpoo[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = poopstain_icon, "layer" = MOB_LAYER)
	...
	if (gloves)
		...
		if (gloves.blood_DNA)
			...
			overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
		if (src.gloves.poop_covering)
			var/icon/poopstain_icon = icon('pooeffect.dmi', "poohands[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = poopstain_icon, "layer" = MOB_LAYER)
		if (src.gloves.egg_covering)
			var/icon/eggstain_icon = icon('eggeffect.dmi', "egghands[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = eggstain_icon, "layer" = MOB_LAYER)
	else if (blood_DNA)
		var/icon/stain_icon = icon('blood.dmi', "bloodyhands[!lying ? "" : "2"]")
		overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	else if (src.pickeduppoo)
		var/icon/poopstain_icon = icon('pooeffect.dmi', "poohands[!src.lying ? "" : "2"]")
		src.overlays += image("icon" = poopstain_icon, "layer" = MOB_LAYER)
	...
	if (shoes)
		...
		if (src.shoes.poop_covering)
			var/icon/poopstain_icon = icon('pooeffect.dmi', "shoepoo[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = poopstain_icon, "layer" = MOB_LAYER)
		if (src.shoes.egg_covering)
			var/icon/eggstain_icon = icon('eggeffect.dmi', "shoeegg[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = eggstain_icon, "layer" = MOB_LAYER)
/*	if (w_radio)
		...
		if (istype(wear_mask, /obj/item/clothing/mask))
			...
			if (!istype(wear_mask, /obj/item/clothing/mask/cigarette))
				if (wear_mask.blood_DNA)
					...
					overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
				if (src.wear_mask.poop_covering)
					var/icon/poopstain_icon = icon('pooeffect.dmi', "maskpoo[!src.lying ? "" : "2"]")
					src.overlays += image("icon" = poopstain_icon, "layer" = MOB_LAYER)
				if (src.wear_mask.egg_covering)
					var/icon/eggstain_icon = icon('eggeffect.dmi', "maskegg[!src.lying ? "" : "2"]")
					src.overlays += image("icon" = eggstain_icon, "layer" = MOB_LAYER)
	...
	if (wear_suit)
		...
		if (wear_suit)
			if (wear_suit.blood_DNA)
				...
				overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
			if (src.wear_suit.poop_covering)
				var/icon/poopstain_icon = null
				if (istype(wear_suit, /obj/item/clothing/suit/armor/vest || /obj/item/clothing/suit/wcoat || /obj/item/clothing/suit/armor/a_i_a_ptank))
					poopstain_icon = icon('pooeffect.dmi', "armorpoo[!src.lying ? "" : "2"]")
				else if (istype(wear_suit, /obj/item/clothing/suit/det_suit || /obj/item/clothing/suit/labcoat))
					poopstain_icon = icon('pooeffect.dmi', "coatpoo[!src.lying ? "" : "2"]")
				else
					poopstain_icon = icon('pooeffect.dmi', "suitpoo[!src.lying ? "" : "2"]")
				src.overlays += image("icon" = poopstain_icon, "layer" = MOB_LAYER)
			if (src.wear_suit.egg_covering)
				var/icon/eggstain_icon = null
				if (istype(wear_suit, /obj/item/clothing/suit/armor/vest || /obj/item/clothing/suit/wcoat || /obj/item/clothing/suit/armor/a_i_a_ptank))
					eggstain_icon = icon('eggeffect.dmi', "armoregg[!src.lying ? "" : "2"]")
				else if (istype(wear_suit, /obj/item/clothing/suit/det_suit || /obj/item/clothing/suit/labcoat))
					eggstain_icon = icon('eggeffect.dmi', "coategg[!src.lying ? "" : "2"]")
				else
					eggstain_icon = icon('eggeffect.dmi', "suitegg[!src.lying ? "" : "2"]")
				src.overlays += image("icon" = eggstain_icon, "layer" = MOB_LAYER)
		...
		if (istype(wear_suit, /obj/item/clothing/suit/straight_jacket))
			...
			if ((l_hand || r_hand))
				...
				drop_item()
				hand = h
		if ((istype(src.wear_suit, /obj/item/clothing/suit/space/rig)) && (src.healths) && (!src.lying))
			if (src.stat != 2)
				var/icon/rigindicator_icon = null
				switch(health)
					if(80 to 99)
						rigindicator_icon = icon('suit.dmi', "rigsuithealth1")
					if(60 to 80)
						rigindicator_icon = icon('suit.dmi', "rigsuithealth2")
					if(40 to 60)
						rigindicator_icon = icon('suit.dmi', "rigsuithealth3")
					if(20 to 40)
						rigindicator_icon = icon('suit.dmi', "rigsuithealth4")
					if(0 to 20)
						rigindicator_icon = icon('suit.dmi', "rigsuithealth5")
				src.overlays += image("icon" = rigindicator_icon, "layer" = MOB_LAYER)

	// Updating hair before the helmet
	if(!face_standing || !face_lying)
		update_face()
	if(!stand_icon || !lying_icon)
		update_body()
	...
	/* Hair appears on top of clothes but doesn't disappear when you lay down. Fix later.
	if (!lying || !mutantrace)
		if(istype(head,/obj/item/clothing/head) || istype(src.wear_suit, /obj/item/clothing/suit/space) || istype(wear_suit, /obj/item/clothing/suit/bio_suit))
			if (gender == MALE)
				overlays += image("icon" = 'human_face.dmi', "icon_state" = "hair_a_s", "layer" = MOB_LAYER)
			else if (gender == FEMALE)
				overlays += image("icon" = 'human_face.dmi', "icon_state" = "hair_pony_s", "layer" = MOB_LAYER)
		else
			overlays += image("icon" = 'human_face.dmi', "icon_state" = "[hair_icon_state]_s", "layer" = MOB_LAYER+0.1)
			overlays += image("icon" = 'human_face.dmi', "icon_state" = "[face_icon_state]_s", "layer" = MOB_LAYER+0.1)*/
	...
	if (head)
		...
		if (head.blood_DNA)
			...
			overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
		if (src.head.poop_covering)
			var/icon/poopstain_icon = icon('pooeffect.dmi', "helmetpoo[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = poopstain_icon, "layer" = MOB_LAYER)
		if (src.head.egg_covering)
			var/icon/eggstain_icon = icon('eggeffect.dmi', "helmetegg[!src.lying ? "" : "2"]")
			src.overlays += image("icon" = eggstain_icon, "layer" = MOB_LAYER)
	...
	if (bodytemperature < 100 && frozen < 15 && !mind.special_role == "Changeling")
		if(mutations & !COLD_RESISTANCE)
			src.frozen++ // you're going to need help when you're this fucking cool, no turning back.

	if (frozen > 15)
		frozen = 15 // F�hrerin Erika: and then they're like omg im frozen cant unfreeze
					//	F�hrerin Erika: so we have been adminfixing people


	if (frozen) // cool
		if (!lying)
			src.overlays += image("icon" = 'mob.dmi', "icon_state" = "frozen", "layer" = MOB_LAYER)
		else
			src.overlays += image("icon" = 'mob.dmi', "icon_state" = "frozen_l", "layer" = MOB_LAYER)
		src.stunned = 1
		src.stuttering = 1
		src.canmove = 0
		src.flaming = 0
	...
	if (flaming) // On fire
		if (frozen)
			src.frozen = 0
		var/icon/flaming_icon = null
		if (!lying)
			switch(flaming)
				if(1 to 30)
					flaming_icon = icon('mob.dmi', "flaming1")
				if(30.1 to 60)
					flaming_icon = icon('mob.dmi', "flaming2")
				if(60.1 to INFINITY)
					flaming_icon = icon('mob.dmi', "flaming3")
		else
			switch(flaming)
				if(1 to 30)
					flaming_icon = icon('mob.dmi', "flaming1_l")
				if(30.1 to 60)
					flaming_icon = icon('mob.dmi', "flaming2_l")
				if(60.1 to INFINITY)
					flaming_icon = icon('mob.dmi', "flaming3_l")
		src.overlays += image("icon" = flaming_icon, "layer" = MOB_LAYER)
...
/mob/living/carbon/human/proc/update_face()
	...
	//This makes hair disappear when wearing clothes.
	if(istype(head,/obj/item/clothing/head/rabbitears) || istype(head,/obj/item/clothing/head/cowears) || istype(head, /obj/item/clothing/head/kitty) || istype(head, /obj/item/clothing/head/helmet/nernhat) || istype(wear_mask, /obj/item/clothing/mask/gas/mime) || istype(wear_mask, /obj/item/clothing/mask/gas/clown_hat)) //Exclusions (bunny ears etc)
		hair_s = new/icon("icon" = 'human_face.dmi', "icon_state" = "[hair_icon_state]_s")
		hair_l = new/icon("icon" = 'human_face.dmi', "icon_state" = "[hair_icon_state]_l")
	else
		if(istype(head,/obj/item/clothing/head) || istype(src.wear_suit, /obj/item/clothing/suit/space) || istype(wear_suit, /obj/item/clothing/suit/bio_suit)) //Inclusions
			if (gender == MALE)
				hair_s = new/icon("icon" = 'human_face.dmi', "icon_state" = "hair_a_s")
				hair_l = new/icon("icon" = 'human_face.dmi', "icon_state" = "hair_a_l")
			else if (gender == FEMALE)
				hair_s = new/icon("icon" = 'human_face.dmi', "icon_state" = "hair_pony_s")
				hair_l = new/icon("icon" = 'human_face.dmi', "icon_state" = "hair_pony_l")

	//We don't want ponytails sticking out of helmets or hoods.
	if(istype(head,/obj/item/clothing/head/rabbitears) || istype(head,/obj/item/clothing/head/cowears) || istype(head, /obj/item/clothing/head/kitty) || istype(head, /obj/item/clothing/head/helmet/nernhat) || istype(wear_mask, /obj/item/clothing/mask/gas/mime) || istype(wear_mask, /obj/item/clothing/mask/gas/clown_hat)) //Exclusions (bunny ears etc)
		hair_s = new/icon("icon" = 'human_face.dmi', "icon_state" = "[hair_icon_state]_s")
		hair_l = new/icon("icon" = 'human_face.dmi', "icon_state" = "[hair_icon_state]_l")
	else
		if(istype(head,/obj/item/clothing/head/helmet) || istype(head,/obj/item/clothing/head/bio_hood) || istype(head,/obj/item/clothing/head/chaplain_hood) || istype(head,/obj/item/clothing/head/bomb_hood))
			hair_s = new/icon("icon" = 'human_face.dmi', "icon_state" = "hair_a_s")
			hair_l = new/icon("icon" = 'human_face.dmi', "icon_state" = "hair_a_l")
	...
	//To make sure the layers are correct, this makes hair not appear on top of clothes if they're wearing a helmet or a hood, but appears on top of clothes if they're not wearing one.
	if(istype(head,/obj/item/clothing/head/rabbitears) || istype(head,/obj/item/clothing/head/cowears) || istype(src.wear_suit, /obj/item/clothing/head/kitty) || istype(wear_suit, /obj/item/clothing/head/helmet/nernhat) || istype(wear_mask, /obj/item/clothing/mask/gas/mime) || istype(wear_mask, /obj/item/clothing/mask/gas/clown_hat)) //Exclusions (bunny ears etc)
		face_standing.layer = MOB_LAYER-0
	else
		if(istype(head,/obj/item/clothing/head) || istype(wear_mask, /obj/item/clothing/mask) || istype(src.wear_suit, /obj/item/clothing/suit/space) || istype(wear_suit, /obj/item/clothing/suit/bio_suit))
			face_standing.layer = MOB_LAYER-0
		else
			face_standing.layer = MOB_LAYER+0.1

	//Correct hair layering by aerika do not steal.
								...
								if("pockets")
									for(var/obj/item/weapon/mousetrap/MT in  list(target.l_store, target.r_store))
										if(MT.armed)
											...
											MT.layer = OBJ_LAYER
											return
									for(var/obj/item/weapon/reagent_containers/food/snacks/boiledspagetti/SP in  list(target.l_store, target.r_store))
										for(var/mob/O in viewers(target, null))
											if(O == source)
												O.show_message(text("\red <B>You reach into the [target]'s pockets, spaghetti falls out.</B>"), 1)
											else
												O.show_message(text("\red <B>[source] reaches into [target]'s pockets, spaghetti falls out.</B>"), 1)
										target.u_equip(SP)
										if (target.client)
											target.client.screen -= SP
										SP.loc = source.loc
										SP.layer = OBJ_LAYER
										return
									for(var/obj/item/weapon/reagent_containers/food/snacks/meatballspagetti/SP in  list(target.l_store, target.r_store))
										for(var/mob/O in viewers(target, null))
											if(O == source)
												O.show_message(text("\red <B>You reach into the [target]'s pockets, spaghetti falls out.</B>"), 1)
											else
										...
										SP.layer = OBJ_LAYER
										return
									for(var/obj/item/weapon/reagent_containers/food/snacks/spagetti/SP in  list(target.l_store, target.r_store))
										for(var/mob/O in viewers(target, null))
											if(O == source)
												O.show_message(text("\red <B>You reach into the [target]'s pockets, spaghetti falls out.</B>"), 1)
											else
...
/mob/living/carbon/human/show_inv(mob/user as mob)
	...
	<BR><B>Right Hand:</B> <A href='?src=\ref[src];item=r_hand'>[(r_hand ? r_hand : "<font color=red>Nothing</font>")]</A>
	<BR><B>Gloves:</B> <A href='?src=\ref[src];item=gloves'>[(gloves ? gloves : "<font color=red>Nothing</font>")]</A>
	<BR><B>Eyes:</B> <A href='?src=\ref[src];item=eyes'>[(glasses ? glasses : "<font color=red>Nothing</font>")]</A>
	<BR><B>Ears:</B> <A href='?src=\ref[src];item=ears'>[(ears ? ears : "<font color=red>Nothing</font>")]</A>
	<BR><B>Head:</B> <A href='?src=\ref[src];item=head'>[(head ? head : "<font color=red>Nothing</font>")]</A>
	<BR><B>Shoes:</B> <A href='?src=\ref[src];item=shoes'>[(shoes ? shoes : "<font color=red>Nothing</font>")]</A>
	<BR><B>Belt:</B> <A href='?src=\ref[src];item=belt'>[(belt ? belt : "<font color=red>Nothing</font>")]</A>
	<BR><B>Uniform:</B> <A href='?src=\ref[src];item=uniform'>[(w_uniform ? w_uniform : "<font color=red>Nothing</font>")]</A>
	<BR><B>(Exo)Suit:</B> <A href='?src=\ref[src];item=suit'>[(wear_suit ? wear_suit : "<font color=red>Nothing</font>")]</A>
	<BR><B>Back:</B> <A href='?src=\ref[src];item=back'>[(back ? back : "<font color=red>Nothing</font>")]</A> [((istype(wear_mask, /obj/item/clothing/mask) && istype(back, /obj/item/weapon/tank) && !( internal )) ? text(" <A href='?src=\ref[];item=internal'>Set Internal</A>", src) : "")]
	<BR><B>ID:</B> <A href='?src=\ref[src];item=id'>[(wear_id ? wear_id : "<font color=red>Nothing</font>")]</A>
	<BR><B>Suit Storage:</B> <A href='?src=\ref[src];item=s_store'>[(s_store ? s_store : "<font color=red>Nothing</font>")]</A>
