...
/mob/living/silicon/robot/proc/pick_module()
	...
	switch(mod)
		...
		if("Service")
			...
			var/icontype = input("Select an icon!", "Robot", null, null) in list("Waitress", "Bro", "Butler", "Kent", "Butt", "Tupac's Hologram")
			if(icontype== "Waitress")
				...
				icon_state = "Brobot"
			else if(icontype == "Butt")
				icon_state = "buttborg"
			else if(icontype == "Tupac's Hologram")
				icon_state = "tupacshologram"
				real_name = "Tupac's Hologram [pick(rand(1, 999))]"
				overlays -= "eyes"
			else
				icon_state = "Service2"
			modtype = "Butler"
...
//			icon_state = "MedBot"
		...
		if("Security")
			...
			var/icontype = input("Select an icon!", "Robot", null, null) in list("Standard", "Armored", "Robocop", "Robocop Red", "Superjail")
			if(icontype == "Armored")
				icon_state = "Security"
