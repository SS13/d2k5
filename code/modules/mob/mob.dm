			...
			if (state >= 3)
				if (!( killing ))
					for(var/mob/O in viewers(assailant, null))
						O.show_message(text("\red [] has temporarily tightened their grip on []!", assailant, affecting), 1)
						//Foreach goto(97)
			...
			if (state < 2)
				...
				if (state < 3)
					if(istype(affecting, /mob/living/carbon/human))
						...
						*/
					for(var/mob/O in viewers(assailant, null))
						O.show_message(text("\red [] has reinforced their grip on [] (now neck)!", assailant, affecting), 1)
						affecting.emote("choke")
					...
					if (state >= 3)
						...
						if (killing)
							for(var/mob/O in viewers(assailant, null))
								O.show_message(text("\red [] has tightened their grip on []'s neck!", assailant, affecting), 1)
	...
	switch(name)
				...
				if (usr.internal)
					...
					if (!( istype(usr.wear_mask, /obj/item/clothing/mask) ))
						...
						if (usr.internal)
							...
							playsound(usr.loc, 'oxygen_desk.ogg', 20, 1)
							if (usr.internals)
								usr.internals.icon_state = "internal1"
		if("pull")
			usr.pulling = null
			usr << "\blue You have now stopped pulling."
		if("sleep")
		...
		if("throw")
			if (!usr.stat && isturf(usr.loc) && !usr.restrained())
				...
				src << "\blue You are now ready to throw the item you're holding."
		if("drop")
			usr.drop_item_v()
		if("swap")
			usr:swap_hand()
		...
		if("resist")
			...
			if((istype(usr:wear_suit, /obj/item/clothing/suit/straight_jacket)) && usr:canmove && (usr.last_special <= world.time))
				usr.next_move = world.time + 100
				usr.last_special = world.time + 100
				if(usr.mutations & HULK)
					usr << "\green You attempt to break the straight jacket. (This will take around 10 seconds and you need to stand still)"
					for(var/mob/O in viewers(usr))
						O.show_message(text("\red <B>[] is trying to break the straight jacket!</B>", usr), 1)
					spawn(0)
						if(do_after(usr, 50))
							if(!istype(usr:wear_suit, /obj/item/clothing/suit/straight_jacket)) return
							for(var/mob/O in viewers(usr))
								O.show_message(text("\red <B>[] manages to break the straight jacket!</B>", usr), 1)
							usr << "\green You successfully break your straight jacket."
							del(usr:wear_suit)
							usr:wear_suit = null
				else
					usr << "\red You attempt to remove your straight jacket. (This will take around 3 minutes and you need to stand still)"
					for(var/mob/O in viewers(usr))
						O.show_message(text("\red <B>[] attempts to remove the straight jacket!</B>", usr), 1)
					spawn(0)
						if(do_after(usr, 1800))
							if(!istype(usr:wear_suit, /obj/item/clothing/suit/straight_jacket)) return
							for(var/mob/O in viewers(usr))
								O.show_message(text("\red <B>[] manages to remove the straight jacket!</B>", usr), 1)
							usr << "\blue You successfully remove your straight jacket."
							del(usr:wear_suit)
							usr:wear_suit:loc = usr:loc
			...
			if((usr:flaming && usr:lying) || (usr:flaming && usr:resting))
				for(var/mob/O in viewers(usr, null))
					O.show_message(text("\red <B>[] rolls around on the floor, trying to extinguish the flames!</B>", usr), 1)
				if(do_after(usr, 30))
					usr:flaming -= 8

			if(istype(usr:wear_suit, /obj/item/clothing/suit/straight_jacket) && (usr.last_special <= world.time) && usr:buckled)
				usr.next_move = world.time + 100
				usr.last_special = world.time + 100
...
/mob/verb/changes()
	...
	if (client)
		...
		src << browse('changelog.html', "window=changes;size=500x600")
		client.changes = 1
...
/mob/verb/observe()
	...
	for (var/obj/machinery/singularity/S in machines)
...
//					world << "comparing [B.current] to [eye]"
					if (B.current == eye)
						for (var/mob/living/silicon/decoy/D in mobz)
							if (eye)
								eye = D
		if (eye)
			client.eye = eye
	...
	if(href_list["priv_msg"])
		...
		if(M)
			...
			if((usr.client && usr.client.holder) || !(M.client && M.client.holder && M.client.stealth))
				t = strip_html(input("Message:", text("Private message to [M.key]")) as text ,1024)
			else
				t = strip_html(input("Message:", text("Private message to Administrator"))  as text ,1024)
...
/atom/movable/verb/pull()
	...
	if (!( anchored ))
		usr.pulling = src
		usr << "\blue You are now pulling the [src]"
		if(ismob(src))
			var/mob/M = src
...
/client/New()
	if(findtextEx(key, "Telnet @"))
		...
		del(src)
	if(userisjobanned(src) != 0)
		var/isbanned = CheckBan(src)
		if (isbanned)//4testing
			var/banlength = GetBanLength(src)
			src << browse("<meta http-equiv=\"REFRESH\" content=\"0;url=[isbanned]\">", "window=mapwindow.titalscreen")
			winshow(src, "mapwindow.titalscreen", 1)
			winshow(src, "window=mapwindow.titalscreen", 1)

			if(banlength != "Permanent")
				log_access("Prisoner Login: [src] ([src.address] / [src.computer_id])")
				message_admins("\blue Prisoner Login: [src] ([src.address] / [src.computer_id])")
				src.prisoner = 1
				src.muted = 1
				src.clear_admin_verbs()
				src.update_admins(null)
			else
				log_access("Failed Login: [src] ([src.address] / [src.computer_id]) - Permabanned")
				message_admins("\blue Failed Login: [src] ([src.address] / [src.computer_id]) - PermaBanned")
				sleep(1)
				del(src)
...
// Typo from the oriignal Robustmin here, below lies the jitteriness process. So make of his code what you will, the previous comment here was just a copypaste of the above.
...
/mob/Stat()
	...
	if (src.client)
		...
		if(src.emotetime > 0)
			stat(null, "Emote Cooldown: [emotetime] seconds")
			spawn(10)
				emotetime--
		if(src.chattime > 0)
			stat(null, "Chat Cooldown: [emotetime] seconds")
			spawn(10)
				chattime--
		stat(null, "")
	if (spell_list.len)
	...
	var/playercount = 0
	statpanel("Player List")
	for (var/mob/M in mobz)
		if(M.client && M.key)
			playercount++
			if(M.client.stealth)
				stat("Player", "[M.client.fakekey]")
			else
				if(M.client.holder && M.client.holder.rank)
					stat("[M.client.holder.rank]", "[M.key]")
				else
					stat("Player", "[M.key]")
	stat("", "")
	stat("Total Players:", "[playercount]")
	playercount = 0
