...
/obj/admins/proc/player()
	...
	dat += "}"
	dat += "</style></head>"
	dat += "<body><table border=1 cellspacing=0><B><tr><th>Name</th><th>Key</th><th>Options</th><th>PM</th><th>Traitor?</th></tr></B>"
	...
	for(var/mob/M in mobs)
		if(M.ckey)
			...
			if(isalien(M))
				dat += "Alien"
			dat += ")</td>"
			dat += {"<td>[(M.client ? "[M.client]" : "No client")] <a href='http://www.reputationauthority.org/lookup.php?ip=[M.lastKnownIP]' target='_blank'>[M.lastKnownIP]</a></td>
