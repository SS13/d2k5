...
/client/proc/update_admins(var/rank)
	...
	switch (rank)
		if ("Host")
			...
			verbs += /client/proc/disablecolorooc // -- Urist
...
// 			verbs += /client/proc/intercom_view 			//-errorage
			...
			verbs += /client/proc/adminsmoke
			verbs += /client/proc/radiotalk
		...
		//	verbs += /client/proc/cmd_admin_dress
			...
			verbs += /client/proc/jumptocoordinates
...
//			verbs += /client/proc/getmobs
		...
		if ("Badmin")
			...
			verbs += /client/proc/cmd_admin_mute
			verbs += /client/proc/cmd_admin_pm
			verbs += /client/proc/cmd_admin_direct_narrate 	// you're welcome zmyxtr --DS
			verbs += /client/proc/cmd_admin_say
			verbs += /client/proc/cmd_admin_subtle_message
			//verbs += /client/proc/cmd_admin_world_narrate 	// -- TLE
			//verbs += /client/proc/get_admin_state
...
/client/proc/disablecolorooc()
	set category = "Fun"
	set name = "Reset OOC Text Color (for stealth modes)"
	ooccolor = ""
	return
...
/client/proc/stealth()
	...
	stealth = !stealth
	if(stealth)
		var/new_key = strip_html(trim(input("Enter your desired display name.", "Fake Key", key)))
		if(!new_key)
			stealth = 0
			return
...
/client/proc/warn(var/mob/M in mobz)
...
/client/proc/adminsmoke()
	set category = "Special Verbs"
	set name = "Create Smoke"
	set desc = "Cause smoke for spawning bananas, or stuff."
	if(!holder)
		src << "Only administrators may use this command."
		return

	var/list/smokes = list("Bad smoke", "Normal smoke") // STEAL CODE
	var/choicess = input("What kind of smoke?") in smokes
	switch(choicess)
		if(null)
			return 0
		if("Bad smoke")
			var/bsmokeamount = input("How much smoke, under 20!:") as num
			if(bsmokeamount > 20)
				alert("get a fucking nicotine patch")
			if(bsmokeamount < 0)
				alert("are you deliberately trying to fucking BREAK shit?")
			else
				var/datum/effects/system/bad_smoke_spread/smoke = new /datum/effects/system/bad_smoke_spread()
				smoke.set_up(bsmokeamount, 0, usr.loc)
				smoke.start()
				message_admins("\blue [ckey] creating bad smoke at [usr.loc] with a thickness of [bsmokeamount].")
		if("Normal smoke")
			var/gsmokeamount = input("How much smoke, under 20!:") as num
			if(gsmokeamount > 20)
				alert("get a fucking nicotine patch")
			if(gsmokeamount < 0)
				alert("are you deliberately trying to fucking BREAK shit?")
				...
				smoke.set_up(gsmokeamount, 0, usr.loc)
				smoke.start()
				message_admins("\blue [ckey] creating smoke at [usr.loc] with a thickness of [gsmokeamount].")
...
*/
/client/proc/give_spell(mob/T as mob in mobz) // -- Urist
	set category = "Fun"
	set name = "Give Spell"
...
/client/proc/radiotalk()
	set category = "Fun"
	set name = "Talk on Radio"
	set desc = "fuck with people on the radio, but mysteriously."
	if(!holder)
		src << "Only administrators may use this command."
		return
	var/aradiosender = input("Who sends the message?") as text
	var/aradiomessage = input("What message?") as text
	var/aradiochannel = input("What channel? Use shit like Engineering, Command, Common, Security etc.") as text
	if(aradiosender && aradiomessage && aradiochannel)
		radioalert(aradiomessage,aradiosender,aradiochannel)
	else
		return
...
/client/proc/make_sound(var/obj/O in world) // -- TLE
	...
	if(O)
		var/message = strip_html(input("What do you want the message to be?", "Make Sound") as text|null)
		if(!message)
			return
...
/client/proc/stealthadmin()
	...
	switch (holder.rank)
		...
		if ("Goodmin") //Former Primary Administrator
			if(holder.state == 2) // if observing
				// Settings
				verbs += /obj/admins/proc/toggleaban			//abandon mob
				verbs += /obj/admins/proc/toggleooc				//toggle ooc
		...
		if ("Admin Candidate") //Removed the 'Administrator' rank, has same rights as Goodmin (Expected that these will be set manually each round)
			if(holder.state == 2) // if observing
				// Settings
				verbs += /obj/admins/proc/toggleaban			//abandon mob
				verbs += /obj/admins/proc/toggleooc				//toggle ooc
		...
		if ("Goat Fart") //Former Filthy Xeno
			verbs += /client/proc/cmd_admin_say
			verbs += /client/proc/cmd_admin_gib_self
			verbs += /client/proc/warn
...
/client/proc/changename(var/mob/M in mobz)
	...
	var/newname = strip_html(input(usr,"New Name?","Change Name","[M.name]") as text)
	if(!newname)
		return
	log_admin("[usr] edited [M]'s name to [newname]")
	M.name = newname
	M.real_name = newname
	if(ishuman(M))
		...
		if(H.wear_id)
			...
			if (istype(id,/obj/item/weapon/card/id/))									// Could be a PDA
...
/client/proc/changeckey(var/mob/M in mobz)
	...
	var/newckey = strip_html(input(usr,"New Ckey?","Change Player","[M.ckey]") as text)
	var/newname = strip_html(input(usr,"New Name?","Change Name","[M.name]") as text)
	if(!newname || !ckey)
		return
	log_admin("[usr] edited [M]'s name to [newname] and ckey to [newckey]")
	M.name = newname
	M.real_name = newname
	M.ckey = newckey
	if(ishuman(M))
...
/client/proc/viewfingerprints(var/obj/I in mobz)
	...
	if(isobj(I))
		src << "<b>Fingerprints for [I.name]:</b>"
