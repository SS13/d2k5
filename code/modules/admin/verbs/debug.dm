...
/client/proc/callproc()
	...
	for(i=1, i<argnum+1, i++) // Lists indexed from 1 forwards in byond
		...
		switch(class)
			...
			if("text")
				lst[i] = strip_html(input("Enter new text:","Text",null) as text)
...
/client/proc/cmd_admin_robotize(var/mob/M in mobz)
	set category = "Fun"
	set name = "Make Robot"
...
/client/proc/makepAI(var/turf/T in turfs)
	set category = "Fun"
	set name = "Make pAI"
	set desc = "Specify a location to spawn a pAI device, then specify a key to play that pAI"

	var/list/available = list()
	for(var/mob/C in mobz)
		if(C.key)
			available.Add(C)
	...
	pai.name = strip_html(input(choice, "Enter your pAI name:", "pAI Name", "Personal AI") as text, 35)
...
/client/proc/cmd_admin_alienize(var/mob/M in mobz)
	set category = "Fun"
	set name = "Make Alien"
...
/client/proc/cmd_admin_metroidize(var/mob/M in mobz)
	set category = "Fun"
...
/client/proc/cmd_admin_grantfullaccess(var/mob/M in mobz)
	set category = "Admin"
...
/client/proc/cmd_admin_dress(var/mob/living/carbon/human/M in world)
	...
	switch(dresscode)
		...
		if ("nurse")
			M.equip_if_possible(new /obj/item/clothing/under/schoolgirl2(M), M.slot_w_uniform)
			M.equip_if_possible(new /obj/item/clothing/shoes/red_whitestockings(M), M.slot_shoes)
			M.equip_if_possible(new /obj/item/clothing/suit/labcoat(M), M.slot_wear_suit)
			M.equip_if_possible(new /obj/item/clothing/glasses/regular(M), M.slot_glasses)

		if ("nazi male")
			M.equip_if_possible(new /obj/item/clothing/under/nazi(M), M.slot_w_uniform)
			M.equip_if_possible(new /obj/item/clothing/shoes/jackboots(M), M.slot_shoes)
			M.equip_if_possible(new /obj/item/clothing/suit/nazicoat(M), M.slot_wear_suit)
			//M.equip_if_possible(new /obj/item/clothing/glasses/regular(M), M.slot_glasses)

		if ("nazi female")
			M.equip_if_possible(new /obj/item/clothing/under/nazifem(M), M.slot_w_uniform)
			M.equip_if_possible(new /obj/item/clothing/shoes/boots_wstockings(M), M.slot_shoes)
			M.equip_if_possible(new /obj/item/clothing/suit/nazicoat(M), M.slot_wear_suit)
			//M.equip_if_possible(new /obj/item/clothing/glasses/regular(M), M.slot_glasses)
...
*/
		...
		if("assassin")
			...
			for(var/obj/item/briefcase_item in sec_briefcase)
				del(briefcase_item)
			for(var/i=3, i>0, i--)
				sec_briefcase.contents += new /obj/item/weapon/money
...
// Dismemberment Debug
/client/proc/organchop(var/mob/living/carbon/human/M as mob in mobz)
	set name = "Remove Organ"
	set category = "Debug"
	if(!ishuman(M))
		src << "\red This mob is not a human. No bugging shit up for you."
		return
	var/a = input("Choose an Organ") in list("head", "arm_left", "arm_right", "hand_left", "hand_right", "leg_left", "leg_right", "foot_left", "foot_right")
	message_admins("[src] has removed \"[a]\" from [M]")
	M.removePart(a)
	M << "\red You feel something drop off."
	return

/client/proc/organadd(var/mob/living/carbon/human/M as mob in mobz)
	set name = "Add Robotic Organ"
	set category = "Debug"
	if(!ishuman(M))
		src << "\red This mob is not a human. No bugging shit up for you."
		return
	var/a = input("Choose an Organ") in list("arm_left", "arm_right", "leg_left", "leg_right")
	message_admins("[src] has added a robotic \"[a]\" to [M]")
	M.addRoboPart(a)
	M << "\red You feel sprout."
	return