...
/world/proc/load_admins()
	if (config.old_admins)
		...
		if(!dbcon.IsConnected())
			diary << "Failed to load db admins err [dbcon.ErrorMsg()]\n"
			return
		var/DBQuery/my_query = dbcon.NewQuery("SELECT ckey,rank FROM `admins`")
		if(my_query.Execute())
			while(my_query.NextRow())
				var/rank = convert_ranks(my_query.item[2])
				diary << ("ADMIN: [my_query.item[1]] = [rank]")
				admins[my_query.item[1]] = rank
		dbcon.Disconnect()
		if (!admins)
...
/world/proc/convert_ranks(var/nym as num)
	switch(nym)
		if(0) return "Cluwne"
		if(1) return "Bestmin 2012"
		if(2) return "Goat Fart"
		if(3) return "Shitler"
		if(4) return "Temporary Admin"
		if(5) return "Admin Candidate"
		if(6) return "Goodmin"
		if(7) return "Badmin"
		if(8) return "Robustmin"
		if(9) return "Coder"
		if(10) return "Host"
		else  return 0
