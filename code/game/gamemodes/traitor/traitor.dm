...
/datum/game_mode/proc/auto_declare_completion_traitor()
	for(var/datum/mind/traitor in traitors)
		...
		if(traitor.objectives.len)//If the traitor had no objectives, don't need to process this.
			var/traitorpayment = 100 + text2num(src.getInflation())
			if(doTransaction(traitor.current.ckey,"[traitorpayment]","Successful traitor.") != 1)
				src << "\blue Unfortunately you lack a bank account and didn't get paid for being a successful traitor."
			else
				src << "\blue You got ϛtraitorpayment] for being a successful traitor!"
			var/traitorwin = 1
			var/count = 1
			for(var/datum/objective/objective in traitor.objectives)
