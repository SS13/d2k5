			...
			if (location in escape_zone)
				...
				if(istype(living, /mob/living/carbon/human))
					...
					if(src.doTransaction(living.ckey,"[escapepayment]","Escaped the station alive.") != 1)
						...
						src << "\blue You got ϛescapepayment] for escaping the station alive!"
					for (var/obj/item/weapon/money/LODSEMONE in living:get_contents())
						if(LODSEMONE.value > 0)
							if(src.doTransaction(living.ckey,"[LODSEMONE.value]","Deposited from escape.") != 1)
								...
								src << "\blue Deposited ϛLODSEMONE.value] to your account as you carried this."
...
//IMPOSSIBRU	<B>Final Station Budget:</B> [getBalance("GAYBUTTHOLES")]<BR>"}
	...
	<B>Money Spent:</B> ϛscore_moneyspent]<BR>
	<B>Money Earned:</B> ϛscore_moneyearned]<BR>
	<B>Inflation Taxes:</B> ϛgetInflation()]<BR>
