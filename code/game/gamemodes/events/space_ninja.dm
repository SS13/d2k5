...
*/
	...
	for(var/obj/landmark/L in landmarkz)
		if (L.name == "carpspawn")
			spawn_list.Add(L)
	...
	for(var/mob/living/carbon/alien/humanoid/xeno in mobz)
		if(istype(xeno))
			xeno_list += xeno
...
/client/proc/send_space_ninja()
	...
	var/mission
	while(!mission)
		mission = strip_html(input(src, "Please specify which mission the space ninja shall undertake.", "Specify Mission", ""))
		if(!mission)
...
/obj/item/clothing/suit/space/space_ninja/proc/lock_suit(mob/living/carbon/U, X = 0)
	if(X)//If you want to check for icons.
		...
		if(U.mind.special_role!="Space Ninja")
			U << "\red <B>fÔaL ɈRRoR</B>: 382200-*#00CՄE <B>RED</B>\nUNAUHORIZED USƠDETǃeD\nCoMMǎCING SUB-R0UIN3 13...\nTǒMInATING U-U-USǒ..."
			U.gib()
			return 0
